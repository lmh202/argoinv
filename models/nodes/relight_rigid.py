...
class RelightRigidNodes(RelightableGaussian):
    def __init__(
        self,
        **kwargs
    ):
    ...
    @property
    def get_normal(self):
        return self.transform_normals(self.normal_activation(self._normals))

    @property
    def get_quats(self):
        return self.quat_act(self.transform_quats(self._quats))
    ...
    def transform_normals(self, means: torch.Tensor) -> torch.Tensor:
        """
        transform the means of instances to world space
        according to the pose at the current frame
        """
    ...
    def get_gaussians(self, cam: dataclass_camera) -> Dict[str, torch.Tensor]:
        ...
        # quat = torch.tensor([np.cos(theta / 2), 0, 0, np.sin(theta / 2)]).to(device=self.device)         
        # quat = quat[None,None,...].repeat(20,3, 1).float()
        # self.instances_quats = torch.nn.Parameter(quat_mult(self.instances_quats, quat))

        world_means = self.transform_means(self._means) #- torch.tensor([10,5,-0.25]).to(device=self._means.device)  #insertion #- torch.tensor([-10,4,-0.5]).to(device=self._means.device)
        ...
        #rgbs = rgbs * torch.tensor([255,255,250]).to(device=rgbs.device) / 255 * 1.1
        ...
        activated_normals = self.normal_activation(self._normals)
        world_normals = self.transform_normals(activated_normals)
        ...
        gs_dict = dict(
            ...
            # _incident_dirs = self._incident_dirs,
            # _visibility_tracing = self._visibility_tracing,
            # _incident_areas = self._incident_areas
        )
    ...
    def remove_instances(self, remove_id_list: List[int]) -> None:
        ...
        for ins_ids in remove_id_list:
            ...
            self._normals = Parameter(self._normals[mask])
            self._base_color = Parameter(self._base_color[mask])
            self._roughness = Parameter(self._roughness[mask])
            self._reflectivity = Parameter(self._reflectivity[mask])
            self._sun_visibility = Parameter(self._sun_visibility[mask])
            self._incidents_dc = Parameter(self._incidents_dc[mask])
            self._incidents_rest = Parameter(self._incidents_rest[mask])
        ...
        for id in ids:
            if id not in gaussian_dict:
                instance_raw_dict = {
                    ...
                    "_normals":self._normals[self.point_ids[..., 0] == id],
                    "_base_color":self._base_color[self.point_ids[..., 0] == id],
                    "_roughness":self._roughness[self.point_ids[..., 0] == id],
                    "_reflectivity" :self._reflectivity[self.point_ids[..., 0] == id],
                    "_sun_visibility" :self._sun_visibility[self.point_ids[..., 0] == id],
                    "_incidents_dc":self._incidents_dc[self.point_ids[..., 0] == id],
                    "_incidents_rest":self._incidents_rest[self.point_ids[..., 0] == id],
                }
    ...
    def replace_instances(self, replace_dict: Dict[int, int]) -> None:
        ...
        for ins_id, new_id in replace_dict.items():
            ...
            self._base_color = Parameter(torch.cat([self._base_color, new_gaussian["_base_color"]], dim=0))
            self._roughness = Parameter(torch.cat([self._roughness, new_gaussian["_roughness"]], dim=0))
            self._reflectivity = Parameter(torch.cat([self._reflectivity, new_gaussian["_reflectivity"]], dim=0))
            self._sun_visibility = Parameter(torch.cat([self._sun_visibility, new_gaussian["_sun_visibility"]], dim=0))
            self._incidents_dc = Parameter(torch.cat([self._incidents_dc, new_gaussian["_incidents_dc"]], dim=0))
            self._incidents_rest = Parameter(torch.cat([self._incidents_rest, new_gaussian["_incidents_rest"]], dim=0))
