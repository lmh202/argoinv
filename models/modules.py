...
class PositionalEncoding(torch.nn.Module):
    def __init__(self, num_freqs=3, d_in=3, freq_factor=np.pi, include_input=True):
        super().__init__()
...
class SkyDomeMLP(nn.Module):
    def __init__(self, input_dim=3, hidden_dim=128, output_dim=3, num_layers=3):
        super(SkyDomeMLP, self).__init__()
        layers = []
        # Input layer
...
class EnvLight(torch.nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.shs = torch.nn.Parameter( 
           0.25 * torch.ones(1, 3, 16, requires_grad=True),
            )
        self.sky_intensity_scale = torch.nn.Parameter(torch.ones(3) *3)
        self.road_color = torch.nn.Parameter(torch.ones(3))
        self.sky_mlp = SkyDomeMLP()


    def direct_light(self,dirs,step):
        shape = dirs.shape
        dirs = dirs.reshape(-1, 3)
        #light = self.sky_mlp(dirs) #
        #light = eval_sh(self.deg, self.shs, dirs).clamp_min(0) 
        light = torch.ones_like(dirs) * self.sky_intensity
        #torch.nn.Parameter(torch.tensor([0.6835,0.7401,0.8705])*0.8).to(device=self.sky_intensity.device) #
        # if step > 15000 + 12000:
    ...
    def forward(self, image_infos):
        ...
        light = eval_sh(self.deg, self.shs, l).clamp_min(0) 
        # light = torch.ones_like(l) * self.sky_intensity
        light = light.view(*prefix, -1)

        return light

    def visualize_sky_dome(self, resolution=256):
        theta = np.linspace(0, np.pi, resolution)  # Elevation
        ...
        directions = torch.tensor(directions,dtype=torch.float32).to(device=self.device)
        # Predict RGB values
        with torch.no_grad():
            #colors = self.sky_mlp(directions).detach().cpu().numpy()
            colors = eval_sh(self.deg, self.shs, directions).clamp_min(0) 
        # Reshape for visualization
        colors = colors.reshape((resolution, resolution, 3))
        import cv2
        colors = colors.detach().cpu().numpy()
        rgb_image= cv2.cvtColor(colors, cv2.COLOR_BGR2RGB)
        cv2.imwrite('env.png',rgb_image*255)
