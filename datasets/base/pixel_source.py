...
def resize_sparse_channel(channel, H, W, H1, W1):
    sparse_channel = coo_matrix(channel)
    row_scale = H / H1
    col_scale = W / W1
    new_rows = (sparse_channel.row * row_scale).astype(int)
    new_cols = (sparse_channel.col * col_scale).astype(int)
    resized_sparse_channel = coo_matrix(
        (sparse_channel.data, (new_rows, new_cols)), shape=(H, W)
    )
    return resized_sparse_channel.toarray()
...
class CameraData(object):
    ...
    def create_all_filelist(self):
        ...
        self.shading_filepaths = np.array(shading_filepaths)
        self.load_shading = False
        if os.path.exists(os.path.join(self.data_path, "visibility")):
            self.load_shading = True
    ...
    def load_images(self, load_intensity=True):
        ...
        if load_intensity:
            ...
            for ix, fname in tqdm(
                ...
                row_scale = self.load_size[0]  / intensity.shape[0] #H / H1
                col_scale = self.load_size[1]  / intensity.shape[1]
                new_rows = (sparse_array.row * row_scale).astype(int)
                new_cols = (sparse_array.col * col_scale).astype(int)
                
                # Create a new sparse matrix at the new size
                resized_sparse_array = coo_matrix(
                    (sparse_array.data, (new_rows, new_cols)), shape=(self.load_size[0], self.load_size[1])
                )

                # Convert back to dense array and restore the original 3D shape
                resized_intensity = resized_sparse_array.toarray()[:, :, np.newaxis]

                intensity_images.append(resized_intensity)
        ...
        if os.path.exists(f_path):
            ...
            for ix, fname in tqdm(
                ...
                albedo_images.append(rgb)
            # normalize the images to [0, 1]
            self.albedo_images = albedo_images = torch.from_numpy(np.stack(albedo_images, axis=0)) / 255
            ...
            for ix, fname in tqdm(
                ...
                desc="Loading roughness images",
                dynamic_ncols=True,
                total=len(self.roughness_filepaths),
            ):
                rough = Image.open(fname).convert("RGB")
                # resize them to the load_size
                rough = rough.resize(
                    (self.load_size[1], self.load_size[0]), Image.BILINEAR
                )
                ...
                roughness_images.append(rough[...,0][...,None]/255)
            ...
            if self.load_shading:
                ...
                for ix, fname in tqdm(
                    ...
                    desc="Loading shading images",
                    dynamic_ncols=True,
                    total=len(self.shading_filepaths),
                ):
                    rgb = Image.open(fname).convert("RGB")
                    # resize them to the load_size
                    rgb = rgb.resize(
                    ...
                    shading_images.append(rgb.mean(axis=-1)[...,None]/255)
    ...
    def to(self, device: torch.device):
        ...
        if self.intensity_images is not None:
            self.intensity_images = self.intensity_images.to(device)
        if self.normal_images is not None:
            self.normal_images = self.normal_images.to(device)
        if self.roughness_images is not None:
            self.roughness_images = self.roughness_images.to(device)
        if self.shading_images is not None:
            self.shading_images = self.shading_images.to(device)
        if self.albedo_images is not None:
            self.albedo_images = self.albedo_images.to(device)
    ...
    def get_image(self, frame_idx: int) -> Dict[str, Tensor]:
        ...
        if self.intensity_images is not None:
            intensity_images = self.intensity_images[frame_idx]
            if self.downscale_factor != 1.0:
                intensity_images = (
                    torch.nn.functional.interpolate(
                        intensity_images.unsqueeze(0).permute(0, 3, 1, 2),
                        scale_factor=self.downscale_factor,
                        mode="nearest",
                    )
                    .squeeze(0)
        ...
        if self.normal_images is not None:
            normal_images = self.normal_images[frame_idx]
            if self.downscale_factor != 1.0:
                normal_images = (
                    torch.nn.functional.interpolate(
                        normal_images.unsqueeze(0).permute(0, 3, 1, 2),
                        scale_factor=self.downscale_factor,
                        mode="nearest",
                    )
                    .squeeze(0)
        ...
        if self.roughness_images is not None:
            roughness_images = self.roughness_images[frame_idx]
            if self.downscale_factor != 1.0:
                roughness_images = (
                    torch.nn.functional.interpolate(
                        roughness_images.unsqueeze(0).permute(0, 3, 1, 2),
                        scale_factor=self.downscale_factor,
                        mode="nearest",
                    )
                    .squeeze(0)
        ...
        if self.shading_images is not None:
            shading_images = self.shading_images[frame_idx]
            if self.downscale_factor != 1.0:
                shading_images = (
                    torch.nn.functional.interpolate(
                        shading_images.unsqueeze(0).permute(0, 3, 1, 2),
                        scale_factor=self.downscale_factor,
                        mode="nearest",
                    )
                    .squeeze(0)
        ...
        if self.albedo_images is not None:
            albedo_images = self.albedo_images[frame_idx]
            if self.downscale_factor != 1.0:
                albedo_images = (
                    torch.nn.functional.interpolate(
                        albedo_images.unsqueeze(0).permute(0, 3, 1, 2),
                        scale_factor=self.downscale_factor,
                        mode="nearest",
                    )
                    .squeeze(0)
        ...
        if self.albedo_images is not None:
            _image_infos.update({"albedo_images":albedo_images,"roughness_images":roughness_images})
        if self.intensity_images is not None:
            _image_infos.update({"intensity_images":intensity_images}) #, 
        if self.shading_images is not None:
            _image_infos.update({"shading_images":shading_images})
...
class ScenePixelSource(abc.ABC):
    ...
    def prepare_novel_view_render_data(self, dataset_type: str, traj: torch.Tensor) -> list:
        ...
        if dataset_type == "argoverse":
            # Prefer front-left view for Argoverse if available; fall back to
            # whichever camera is configured (e.g. 1-cam setup with cam 0).
            cam_id = 1 if 1 in self.camera_data else self.camera_list[0]
        else:
